...
The following diagram shows STARK Procurement and its interactions with external systems and users:
...
This C4 Level 1 diagram establishes boundaries: we own the central procurement system; everything outside is a dependency or consumer.

### Container Architecture

Zooming into STARK Procurement, this C4 Level 2 diagram shows the internal containers:
...
This container view maps directly to Domain 5 (Operations) deliverables: the web app, API, database, cache, and Kafka integrations.
...
### Buyer's Morning: A Typical User Journey

This journey map illustrates how a Buyer experiences the system during a typical morning workflow:
...
The satisfaction scores (1-5) reflect typical user experience. Low scores on "Resolve discrepancies" indicate this is a pain point we must design well.
...
The following diagram visualizes the approval decision flow with authority matrix and escalation:
...
This flow ensures every approval decision is traceable, delegations are audited, and stale items are automatically escalated.
...
Before diving into domain details, this diagram shows the complete PR-to-Payment workflow:
...
Each numbered stage maps to specific business rules and screens defined in the domains below.
...
### 1.5 Screen Navigation Map

This diagram shows how users navigate between the 16 screens:
...
The Dashboard (G1) serves as the primary entry point, surfacing action items that route users to the appropriate functional screens.
...
The following ER diagram shows the key entities and their relationships:
...
This model supports the full PR→PO→Invoice lifecycle with comprehensive approval and delegation tracking. Note that USER is consumed from SSO; we store only `userId` references.
...
The following diagram shows the complete topic topology for Kafka-native integration:
...
This architecture provides: (1) isolated failure domains per source, (2) independent monitoring per topic, (3) clear ownership boundaries, and (4) built-in resilience via Kafka's offset management and DLQ patterns.
