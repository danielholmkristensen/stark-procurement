 ...
 * status/supplier filters, pagination, grouped view, and kanban link.
 */

import { useState, useMemo } from "react";
import Link from "next/link";
import { List, Layers, LayoutGrid } from "lucide-react";
...
export function POList() {
  ...
  // Kanban mode redirects to dedicated page
  if (viewMode === "kanban") {
    // We'll redirect, but for now show a link
  }
  ...
  return (
    <tr className={`hover:bg-gray-50 ${getEscalationCardClass(po.escalationLevel).replace('border-', 'border-l-2 border-l-')}`}>
      ...
      <td className="px-4 py-3">
        <LinkedPRsBadge prIds={po.prIds} prs={allPRs || []} />
      </td>
...
function GroupedView({
  ...
  // Get escalation level for section styling
  const getPriorityEscalation = (priority: BusinessPriority) => {
    switch (priority) {
      case "critical":
    ...
    <div className="p-4 space-y-4">
      {PRIORITY_ORDER.map((priority) => {
        ...
        return (
          <CollapsibleSection
            key={priority}
            title={`${getBusinessPriorityLabel(priority).toUpperCase()} — ${getBusinessPriorityDescription(priority)}`}
            count={pos.length}
